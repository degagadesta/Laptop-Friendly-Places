...
export async function loadPlaces() {
    ...
    try {
        ...
        places.forEach(place => {
            ...
            if (isTarget) { map.setView([lat, lng], 16); marker.openPopup(); }

            // Build image tag if available
            // Priority: image_url (Supabase) > image_data (base64) > image (legacy path)
            const imgSrc = place.image_url
                ? place.image_url
                : (place.image_data && place.image_mime)
                    ? `data:${place.image_mime};base64,${place.image_data}`
                    : place.image ? `http://localhost:8000/${place.image}` : null;

            marker.bindPopup(`
                <div class="map-card">
                    ${imgSrc ? `<img src="${imgSrc}" class="map-card-img" alt="${place.name}">` : ''}
