...
async function loadPlaces() {
  ...
  try {
    ...
    loading.innerHTML =
      '<i class="fa-solid fa-exclamation-circle"></i><span>Failed to load places</span>';
  }
}
...
async function displayPlaces(tab) {
  ...
  container.innerHTML =
    '<div class="loading"><i class="fa-solid fa-spinner"></i><span>Loading...</span></div>';
...
function showPlaceDetails(place) {
  ...
  document.getElementById("sheetWifi").textContent = toRating(
    place.wifi_rating,
  ).toFixed(1);
  document.getElementById("sheetRating").textContent =
    getAverageRating(place).toFixed(1);

  const powerEl = document.getElementById("sheetPower");
  if (powerEl) powerEl.textContent = toRating(place.power_rating).toFixed(1);
  const serviceEl = document.getElementById("sheetService");
  if (serviceEl)
    serviceEl.textContent = toRating(place.service_rating).toFixed(1);
