...
async function loadReports(statusFilter = "") {
  try {
    ...
    reports.forEach((report) => {
      ...
      const reporterName =
        report.reported_by_name || report.reported_by || "Anonymous";
      const placeName =
        report.place_name || "Place #" + (report.place_id || "Unknown");
...
function viewReport(reportId) {
  ...
  modal.style.cssText =
    "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.6);display:flex;align-items:center;justify-content:center;z-index:10000;";
      ...
      <div style="background:#f8f9fa;padding:20px;border-radius:10px;margin-bottom:20px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;margin-bottom:15px;">
          ...
          <div>
            ...
            <div style="margin-top:5px;color:#1a1a1a;">${reporterName}</div>
          </div>
        </div>
...
async function deleteReportedPlace(placeId, reportId) {
  if (
    !confirm(
      "Are you sure you want to remove this place? This action will hide it from users and resolve related reports.",
    )
  ) {
    return;
  }
  ...
  try {
    ...
    await ADMIN_API.post("/admin/reports", {
      report_id: reportId,
      status: "resolved",
    });
    showToast("Place removed and report resolved successfully!", "success");
    await loadReports();
  } catch (err) {
