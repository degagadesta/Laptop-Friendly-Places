...
function getAverageRating(place) {
  return (
    (toRating(place.wifi_rating) +
      toRating(place.power_rating) +
      toRating(place.service_rating)) /
    3
  ).toFixed(1);
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(() => {
    if (window.mapsModule?.initializeMaps) {
      ...
      setTimeout(() => {
        if (window.loadPlacesMap) window.loadPlacesMap("all");
      }, 500);
    }
  }, 500);
});
...
async function loadPlaces(searchQuery = "") {
  try {
    ...
    if (!filtered.length) {
      if (noPlaces) noPlaces.style.display = "block";
      return;
    }
...
async function confirmDeletePlace(placeId) {
  try {
    ...
    const response = await ADMIN_API.del("/admin/places/delete", {
      place_id: placeId,
    });
...
function viewPlace(placeId) {
  ...
  const place = allPlaces.find((p) => Number(p.id) === Number(placeId));
  ...
  body.innerHTML = `
    ...
    <p><strong>WiFi:</strong> ${toRating(place.wifi_rating).toFixed(1)}/5 &nbsp;
       <strong>Power:</strong> ${toRating(place.power_rating).toFixed(1)}/5 &nbsp;
       <strong>Service:</strong> ${toRating(place.service_rating).toFixed(1)}/5</p>
